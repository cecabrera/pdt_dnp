...
```{r echo=FALSE, eval=FALSE}
example <- d$HUILA[, .N, keyby = HUILA][order(N, decreasing = T)][1:30,.(HUILA, stemDocument(x = HUILA,  language = "es"), N)]
saveRDS(example, "chunks/example.RDS")
```
```{r example, eval=FALSE}
print(d$HUILA[, .N, keyby = HUILA][order(N, decreasing = T)][1:30,.(HUILA, stemDocument(x = HUILA,  language = "es"), N)])
```
```{r}
print(readRDS("chunks/example.RDS"))
```
...
d <- rbindlist(d, use.names = T)[, sum(f), keyby = .(depart, word)][order(V1, decreasing = T)]
print(d)
```
```{r echo=FALSE, eval=TRUE}
d <- readRDS("chunks/frequencies.RDS")
print(d)
```
...
wellPanel(
  fluidRow(
    ...
    choices = departments, selected = "CESAR", multiple = F)
    , renderPlot(
...
```{r echo=FALSE}
q <- readRDS("chunks/weight_q.RDS")
q_melted <- readRDS("chunks/weight_q_melted.RDS")
```
...
Los algoritmos de k-medias ubican un centroide por cada grupo y conglomera cada documento respecto a su cercanía a un centroide. Es ampliamente utilizado en problemas de datos estructurados. El cálculo del número óptimo de grupos (k) es considerado _NP-Hard_ dados los requerimientos computacionales para calcularlo y por eso existen heurísticas para hallar un óptimo local. 

La mayor implicación en los algoritmos anteriores es que a cada "observación" (en nuestro caso "palabra") es asignado a una única categoría. Sin embargo, como sucede en el lenguaje natural, existen palabras que podrían hacer parte de varias categorías (o tópicos o temas). Por eso, el modelaje de tópicos será el escogido para clasificar los Planes de Desarrollo Departamentales. El modelaje de tópicos es un método de clasificación no-supervisado, útil para encontrar grupos naturales de palabras que aparentemente no tienen relación. 
...
Se convierte la variable a un formato apto para realizar el cálculo del LDA. Se analizarán 2 tópicos potenciales para entender el contenido de palabras en cada uno y, en vista que existen variables aleatorias en el modelo, se define una semilla (1234) para conservar consistencia en los resultados. 
...
Este procedimiento convirtió el modelo a un formato de una fila por tópico por palabra. Para cada combinación, el modelo computa la probabilidad que ese término haya sido generado por ese tópico. Por ejemplo, el término "abandon" tiene una probabilidad de 1.71e-04 de haber sido generado del tópico 1 y una del 3.28e-05 de haber sido generado del tópico 2. 
...
Esta gráfica permite entender los dos tópicos que fueron extraidos de los PDTs. Las palabras más comunes en el tópico 1 incluyen "población", "salud", "implementar", "proces", "indic" lo que puede sugerir que están más enfocados a la implementación de proyectos de salud. Aquellas palabras más comunes en el tópico 2 incluyen "social", "econom", "derech", "genero" y "niñ" infeririendo una inclinación hacia estrategias de interacción social (niñez, género, derecho) ajenas a la salud. Aquí se resalta una ventaja del modelaje de tópicos opuesta a los métodos "tradicionales de conglomerados": los tópicos en lenguaje natural se pueden superponer entre palabras.

Como una alternativa a esta análisis, se podría considerar palabras que tuvieron la mayor diferencia de betas (β) entre tópico 1 y tópico 2. Esta se puede estimar usando el logaritmo del radio entre ambos parámetros. Para limitarlo a un grupo de palabras especialmente relevantes, se filtra por las palabras comunes, como aquellas que tienen una probabilidad (beta) mayor a (1/1000) en al menos un tópico. A continuación las palabras con las mayores diferencias entre los dos tópicos. 
...
Las palabras más comunes en el tópico 1 son: "violenci", "viviend", "trabaj", "cultur", "climat", "urban", "saneamient", "tecnolog", "promocion", "interes", "document" posiblemente abordando más temáticas de salud (urban, violenc, climat, saneamient) y convivencia (cultur, trabaj, viviend, tecnolog). En el tópico 2 están las palabras: "formacion", "fisic", "conserv", "natural", "direccion", "comunitari", "desplaz", "orden", "potencial", "coordin" y "asistent" infiendo soluciones a problemáticas sociales (comunitari, coordin, formacion) y de desplazamiento (fisic, orden, direccion). 
...
#### Tópico 1: Proyectos con enfoque en salud

```{r}
print(head(depart[order(`1`, decreasing = T)], 6))
```
...
```{r}
print(head(depart[order(`2`, decreasing = T)], 6))
```
...
La hipótesis resultante del análisis LDA es que el tópico 1 tiene mayor énfasis en proyectos relacionados a la salud y el tópico 2 hacia proyecto sociales de índole educacional y cultural. Aún cuando existen palabras pegadas, la frecuencia con la que aparecen y su relevancia latente en comparación con los documentos hace que el modelo las tenga en cuenta. Esta hipótesis está sujeta a cambios por la calidad en los datos (palabras unidas).
