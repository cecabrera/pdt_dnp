...
names(d) <- departments
# Almacenamos la variable en memoria para correr el procesamiento una sola vez. 
saveRDS(d,"chunks/chunk3.rds")
```
...
En R, el paquete `tm` brinda una lista de las `stop-words` del español. Veamos las primeras 10 y las últimas 10. 

```{r}
stop_es <- stopwords(kind = "es")
cbind(head(stop_es, 10), tail(stop_es, 10))
```

Las suprimimos de todos los documentos:

```{r}
d <- readRDS("chunks/chunk3.rds")
...
Utilizando el algoritmo de _stemming_ de Porter y tomando como ejemplo las útlimas 30 palabras del Plan de Desarrollo del Huila encontramos las siguientes comparaciones.

```{r}
tail(d$HUILA[, .(HUILA, stemDocument(x = HUILA,  language = "es"))],30)
```
